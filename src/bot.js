...
function createBot() {
  ...
  // Имя листа с пробелами — в кавычках: /bind 1AbC... "ДП | Фасув 16.06" Клиент
  // Пишется прямо в группе клиента — chat_id берётся из неё
  bot.command('bind', async (ctx) => {
    if (!isAdmin(ctx)) return;
    // токены: "в кавычках" считаются одним куском
    const parts = (ctx.match.trim().match(/"[^"]*"|«[^»]*»|\S+/g) || []).map((t) =>
      t.replace(/^["«]|["»]$/g, '')
    );
    if (parts.length < 2) {
      return ctx.reply(
        'Формат: /bind <spreadsheet_id> <имя_листа> [название]\n' +
          ...
          'Имя листа с пробелами — в кавычках: /bind 1AbC...xyz "ДП | Фасув 16.06" Клиент'
      );
    }
